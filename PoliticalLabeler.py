...
class PoliticalLabeler:
    ...
    def aggregate_users(self):
        """Groupe les données par utilisateur et calcule la stabilité."""
    ...
    def define_camps(self, low_q=0.40, high_q=0.80):
        ...
        # Mise à jour du dataframe principal avec les camps
        if 'camp' in self.df.columns:
            self.df = self.df.drop(columns=['camp'])
